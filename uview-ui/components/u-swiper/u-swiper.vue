	...
	>
		...
		>
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				...
				>
					<!-- run away bro -->
					<view v-if="showInfo" class="swiper_title" >{{ item.name }}</view>
					<view v-if="showInfo" class="swiper_sub_title">{{item.level== '-1'?'永久有效':'成长值需达到'+item.money}}</view>
					<view v-if="showInfo&&item.level<currentLevel" class="swiper_desc">已解锁更高等级</view>
					<view v-if="showInfo&&item.level == currentLevel" class="swiper_desc">还差{{sMoney}}成长值可升级{{nextLevel.name}}</view>
					<view v-if="showInfo" class="swiper_tag" :style="{ background: item.level == currentLevel?'linear-gradient(90deg, #5B5B7E, #2E2E41)':item.level< currentLevel?'linear-gradient(90deg, #999DE9, #6A5FCE)':'linear-gradient(90deg, #AE754A, #F0D1BA)' }">{{ item.level == currentLevel ? '当前等级' : item.level< currentLevel?'已开通':'待开通' }}</view>
					<view v-if="showInfo&&item.level == currentLevel" class="swiper_process">
						<view class="swiper_process_item" :style="{ width: (250 * (percent/100)) + 'rpx' }"></view>
					</view>
					<!-- end -->
...
.plusBigTitle {
	color: #fde6ce !important;
}
.plusSubTitle {
	color: #91867a !important;
}
.plusSubDesc {
	color: #fde6ce !important;
}
.plusbtn {
	background: linear-gradient(90deg, #f7d8c5, #fac59c) !important;
...
.swiper_tag {
	...
	padding: 10rpx;
	color: #fff;
	background: linear-gradient(90deg, #5b5b7e, #2e2e41);
	border-radius: 0px 0px 0px 18rpx;
	font-size: 24rpx;
	top: 0;
}
.swiper_btn {
